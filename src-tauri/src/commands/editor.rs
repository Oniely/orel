...
use sqlx::{
    mysql::MySqlConnection, pool::PoolConnection, postgres::PgConnection, sqlite::SqliteConnection,
    Column, Executor, MySql, Postgres, Row, SqlSafeStr, Sqlite, TypeInfo,
};

use super::connection::{AppState, DbPool};
use super::decode::{decode_mysql, decode_pg, decode_sqlite, SqlCell};
...
macro_rules! execute_statement {
    ($fn_name:ident, $connection:ty, $decoder:ident, $primary_columns:ident) => {
        async fn $fn_name(
...
execute_statement!(execute_pg, PgConnection, decode_pg, pg_primary_columns);
execute_statement!(
    execute_mysql,
    MySqlConnection,
    decode_mysql,
